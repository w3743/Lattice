...
2. **10 个规划-执行不一致用例**（§10）：需用户拍板补能力还是收窄承诺。
3. **把正激 production 正式并入 `power_topologies.yaml`**：是否纳入需用户确认
   （会改变默认候选集）。
4. **合并四个重复的结果类**（§11.8.3 的失败尝试）：收益有限、风险已知，
   建议在下次顺手改动 `power.py` 结构时一并做，而不是单独一次高风险手术。
5. **验收域的可视化/汇总**：三个域的记录目前分散在 `report.json` 的不同键下，
