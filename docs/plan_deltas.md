...
### 11.17 MOSFET 尝试与失败诊断（2026-09-12，未合入）

**结论先说：本轮没有交付任何新器件能力。** 覆盖面上缺口最大的仍是"二极管以外的
有源器件一个都没有"，而 MOSFET 的实现**未通过自洽性检查，已全部回退**，
仓库回到 §11.16 的已验证状态（619 passed）。本节记录做了什么、错在哪、下次从哪里
开始，以免重复。

**已完成且可复用的部分**（数值本身经核对，问题在集成层）：

1. **ngspice 参考装置已建立且可用**。ngspice 的 `@m1[id]` 在本机这个构建里
   解析不出向量（`vector @m1[id] is not available`），必须用一个 0 V 电压源串在
   漏极回路里、读 `i(VS)` 来测电流。这一点花了不少时间才定位，下次直接照做。
   采集到的 Level-1 NMOS 参考：`VTO=1.0 KP=1e-4 LAMBDA=0.02 GAMMA=0 PHI=0.6`、
   `W=10u L=1u`，在 `VGS∈{1.5,2.5,3.5}` × `VDS∈[0,5]` 上，实测 drain 电流与
   `Ids = 0.5·KP·(W/L)·(Vgs-VTO)²·(1+λ·Vds)`（饱和区）及
   `Ids = KP·(W/L)·((Vgs-VTO)·Vds - Vds²/2)`（线性区）**逐点吻合到 1e-6 相对**
   （例如 `VGS=3.5, VDS=5` 实测 3.4375e-3 A，公式 3.4375e-3 A）。
   即**模型方程本身是对的，参数映射也是对的**。测试台数据用
   `trapz`/`wrdata` 落在 `%TEMP%\lat_mos7`。
2. **雅可比与残差在真实偏置处是一致的**（有限差分核对）：
   在 `(Vgs=3.5, Vds=5)` 处 `dIds/dVds = 6.25e-5`、`dIds/dVgs = 2.75e-3`，
   与解析导数一致到 1e-9 相对。
3. **多端器件接口设计**（`NonlinearStamp.node_currents` / `node_jacobian`、
   `NonlinearDevice.residual_stamp`）已成型，但随本次一并回退。

**真正卡住的地方**：求解器返回的漏极支路电流与器件自身方程**不一致**——
在 `Vgs=3.5, Vds=5` 的电压源偏置下，器件算出 `Ids = 3.4375e-3 A`，
而 `op.branch_current("VD")` 返回 `1.3375e-2 A`（比值 3.89）。诊断过程中确认：

- 求解器**每一次**盖印得到的器件电流都是 3.4375e-3（直接打印 stamp 的输出）；
- 最终矩阵行与右端项确实是 `[6.25e-5, 2.75e-3, 0, 1] · v = -3.4375e-3`；
- 把这一行代回打印出的解，乘积正好等于右端项，残差为机器零。

也就是说，**"打印出来的矩阵 + 打印出来的解"满足方程，但支路电流却被报成另一个值**。
这说明不一致发生在盖印层与解的后处理之间，而不在模型方程里。八种符号约定
（注入电流 ±、雅可比 ±、以及整体翻转）**全部**无法同时给出正确的
`I_VD = -3.4375e-3` 与 1 kΩ 负载下的 `Vdr = 2.0 V`，因此可以排除"单纯符号写反"。

**下一个具体动作**（留给下一轮，已缩小到很窄的范围）：

1. 先写一个**独立于求解器**的 2×2 牛顿实现（本轮已验证它能在 1 kΩ 负载上收敛到
   正确的 `Vdr = 2.0 V`、`Ids = 3e-3 A`），把它与 `NonlinearMNA.operating_point`
   逐迭代对齐，找出第一个偏离的迭代。关键判据就是上一条那组数字。
2. 复查 `OperatingPoint.branch_current` 与电压源支路在**多端器件**存在时的语义：
   单端器件路径（二极管）已被 ngspice 验证到 1e-5 V，所以差异只可能来自多端路径。
3. **在这一点修好之前不要接入外部验证**：本轮若拿一个自洽性都没通过的实现去对
   ngspice，只会得到"看起来还行"的假象——参考数据明明显示模型是对的，
   需要修的是集成层。

**过程教训（已写进流程）**：本轮在"打印出来的数字互相矛盾"上花了过多时间。
下次遇到同类矛盾，应当**立刻**用独立参考实现逐步对齐，而不是继续读同一段代码。

### 11.18 下一步（按价值排序）

1. **修好多端器件集成层，再补 BJT / MOSFET / 运放 / 电流镜**（§11.17 给了明确的
   起手点和判据）。这是覆盖面缺口最大的一项。
